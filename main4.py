...
def execute_experiment(config, iteration=0):
    ...
    train_iterator = torch.utils.data.DataLoader(
        train_data, batch_size=config.batch_size, shuffle=True, num_workers=4, drop_last=False)
    test_iterator = torch.utils.data.DataLoader(
        test_data, batch_size=config.batch_size, shuffle=False, num_workers=4, drop_last=True)
    bn_calibration_iterator = torch.utils.data.DataLoader(
        train_data, batch_size=config.batch_size, shuffle=False, num_workers=4, drop_last=True)
    ...
    for prune_iter in range(config.start_iter, prune_cycles):
        if prune_iter > 0:
            if config.arch_type == "vgg19_bn" or config.arch_type == "resnet18":
        ...
        global weight_masks_tensor
        if weight_masks is not None:
            weight_masks_tensor = [torch.from_numpy(mask).to(hardware) for mask in weight_masks]
    ...
    for idx, (images, labels) in enumerate(data_iterator):
        ...
        for param_name, param in network.named_parameters():
            if 'weight' in param_name:
                ...
                param.grad.data *= weight_masks_tensor[mask_index]
                mask_index += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arch_type", default="vgg19_bn", type=str,
                        help="fc1 | lenet5 | alexnet | vgg19_bn | resnet18 | densenet121")
    parser.add_argument("--prune_percent", default=35.6, type=int, help="Pruning percent")
    parser.add_argument("--prune_iterations", default=14, type=int, help="Pruning iterations count")
